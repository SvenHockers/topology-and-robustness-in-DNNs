...
def _prune_overrides_where_base_sets_value(overrides: Dict[str, Any], base: Dict[str, Any]) -> Dict[str, Any]:
    ...
    This prevents "fixed overrides" from clobbering explicit config values
    (e.g. baseline configs setting graph.use_topology=false).
    """
    out: Dict[str, Any] = {}
    for k, v in (overrides or {}).items():
        if k in base:
            if isinstance(v, dict) and isinstance(base.get(k), dict):
                ...
                # base explicitly sets this key; do not override
                continue
        else:
            out[k] = v
    return out
...
def _should_ignore(path: Path, *, ignore_globs: Sequence[str], ignore_baseline: bool) -> bool:
    ...
    for pat in ignore_globs:
        # Simple substring glob semantics via fnmatch on posix-normalized path
        import fnmatch
...
def _safe_stem(path: Path) -> str:
    # Avoid awkward output paths for configs like "base_e_0.3 copy.yaml"
    return path.stem.replace(" ", "_")


def compute_study_dir(*, output_root: Path, config_dir: Path, config_path: Path) -> Path:
    """
    Mirror config structure under output root.

    Example:
      config/final/tabular/baseline/base_e_0.1.yaml
        -> <output_root>/tabular/baseline/base_e_0.1/
    """
...
def _best_trial(trials: Sequence[Any]) -> Optional[Dict[str, Any]]:
    ...
    best_val = -float("inf")
    for t in trials:
        # TrialRecord from optimiser.study; we keep it duck-typed here
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # Dataset overrides (fixed for all trials in the batch; useful for VECTOR variants).
    ...
    # Optional: auto-generate figs (requires numpy/matplotlib in the environment).
    p.add_argument("--make-plots", action="store_true", help="Run optimiser.plot_history for each study.")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # Fixed context assumptions (forced for every trial).
    ...
    for i, cfg_path in enumerate(cfgs, start=1):
        ...
        try:
            # Respect explicit YAML values (e.g. baseline configs) unless user forces overrides.
            effective_fixed = fixed_overrides
            if not bool(args.force_fixed_overrides):
                from runners.runner_lib import load_config_any
        ...
        # Optional post-hoc plots
        if bool(args.make_plots) and status == "success":
            try:
                from optimiser.plot_history import main as plot_main
                ...
                # Never fail the batch due to plotting.
                pass
        ...
        # Stream an always-up-to-date JSON summary
    ...
    # Write a compact CSV summary for spreadsheets
